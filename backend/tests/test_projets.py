...
def test_creer_projet(client):
    ...
    response = client.post("/api/projets", json={
...
def test_list_projets_vide(client):
    ...
    response = client.get("/api/projets", headers=_auth_headers(token))
...
def test_list_projets_only_own(client, db_session):
    ...
    client.post("/api/projets", json={"name": "Projet User1"}, headers=_auth_headers(token1))

    # User2 ne voit pas le projet de User1
    response = client.get("/api/projets", headers=_auth_headers(token2))
    assert response.status_code == 200
    assert response.json() == []
...
def test_get_projet_detail(client):
    ...
    create_resp = client.post("/api/projets", json={"name": "Rêve de vacances"}, headers=_auth_headers(token))
    projet_id = create_resp.json()["id"]

    response = client.get(f"/api/projets/{projet_id}", headers=_auth_headers(token))
    assert response.status_code == 200
...
def test_get_projet_autre_utilisateur(client):
    ...
    create_resp = client.post("/api/projets", json={"name": "Projet privé"}, headers=_auth_headers(token1))
    projet_id = create_resp.json()["id"]

    response = client.get(f"/api/projets/{projet_id}", headers=_auth_headers(token2))
...
def test_update_projet(client):
    ...
    create_resp = client.post("/api/projets", json={"name": "Ancien nom"}, headers=_auth_headers(token))
    projet_id = create_resp.json()["id"]

    response = client.put(f"/api/projets/{projet_id}", json={"name": "Nouveau nom"}, headers=_auth_headers(token))
    assert response.status_code == 200
...
def test_delete_projet(client):
    ...
    create_resp = client.post("/api/projets", json={"name": "A supprimer"}, headers=_auth_headers(token))
    projet_id = create_resp.json()["id"]

    response = client.delete(f"/api/projets/{projet_id}", headers=_auth_headers(token))
    assert response.status_code == 204

    # Le projet n'existe plus
    get_response = client.get(f"/api/projets/{projet_id}", headers=_auth_headers(token))
...
def test_ajouter_activite_au_projet(client, db_session):
    ...
    create_resp = client.post("/api/projets", json={"name": "Avec activités"}, headers=_auth_headers(token))
    projet_id = create_resp.json()["id"]

    response = client.post(f"/api/projets/{projet_id}/activites", json={
        "activity_id": poi.id,
        "day_number": 2,
    }, headers=_auth_headers(token))
    assert response.status_code == 201
...
def test_ajouter_activite_inexistante(client):
    ...
    create_resp = client.post("/api/projets", json={"name": "Projet test"}, headers=_auth_headers(token))
    projet_id = create_resp.json()["id"]
...
def test_supprimer_activite_du_projet(client, db_session):
    ...
    create_resp = client.post("/api/projets", json={"name": "Projet retrait"}, headers=_auth_headers(token))
    projet_id = create_resp.json()["id"]

    add_resp = client.post(f"/api/projets/{projet_id}/activites", json={"activity_id": poi.id}, headers=_auth_headers(token))
    item_id = add_resp.json()["items"][0]["id"]

    response = client.delete(f"/api/projets/{projet_id}/activites/{item_id}", headers=_auth_headers(token))
    assert response.status_code == 204

    # L'item est bien retiré
    get_resp = client.get(f"/api/projets/{projet_id}", headers=_auth_headers(token))
